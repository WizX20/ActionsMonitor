...
- **PR mode** (`mode: "pr"`) — one row per active PR the authenticated user authored. When a branch has multiple PRs (e.g. hotfix → acceptance and hotfix → production), each PR gets its own row. Uses `PRWorkflowPoller`. Rows are created dynamically and auto-removed after `pr_stale_after` (default `"5m"`). Shows a colour-escalating staleness badge based on `staleness_thresholds`. Supports `ignore_workflows` (sibling of `extra_workflows`) to filter noisy / shadow-IT workflow runs out of row status aggregation and/or notification transition tracking — entries are either bare filenames (drop both) or dicts with `file:` plus `status:` / `notifications:` toggles (default true each). Matching is by workflow-file basename.
